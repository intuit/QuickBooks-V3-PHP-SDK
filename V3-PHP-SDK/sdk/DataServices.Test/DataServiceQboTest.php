...
class DataServiceQboTest extends PHPUnit_Framework_TestCase
{
        ...
        private function prepareDataServiceQBO()
        {
          ...
          */
         public function testDownloadPDF_SalesReceipt_Null()
         {
             $entity = new IPPSalesReceipt();
             $this->invokeDownloadPDF($entity);
         }

         /**
          * @expectedException IdsException
          * @expectedExceptionMessage Operation DownloadPDF is not allowed for entity IPPPurchase
          */
         public function testDownloadPDF_SalesReceipt_WrongType()
         {
         ...
         public function testDownloadPDF_SalesReceipt()
         {
             $entity = new IPPSalesReceipt();
             $entity->Id = 123;
             $expected = "This is tests response";
             $r = $this->invokeDownloadPDF($entity, $this->returnValue(array(200,$expected)));
             $this->assertEquals($expected, $r->getContent());
         }

         public function testDownloadPDF_Exception()
         {
             $entity = new IPPSalesReceipt();
             $entity->Id = 123;
             //$this->assertNull( $this->invokeDownloadPDF($entity, $this->throwException(new Exception("test"))));
             $this->assertNotNull( $this->invokeDownloadPDF($entity, NULL));
         }


         public function testDownloadPDF_URI()
         {
           $this->invokeRestHandlerViaDownloadPDF("ResourceUri", "company/myfakerealm/salesreceipt/123/pdf");
         }

           public function testDownloadPDF_Verb()
         {
             $this->invokeRestHandlerViaDownloadPDF("HttpVerbType", "GET");
         }

         public function testDownloadPDF_ContenType()
         {
             $this->invokeRestHandlerViaDownloadPDF("ContentType", "application/pdf");
         }

         public function testDownloadPDF_ProcessingTemp()
         {
            $contentWriterMock = $this->getMock("ContentWriter");
            $contentWriterMock->expects($this->once())
                            ->method('saveTemp');
            $contentWriterMock->expects($this->once())
                            ->method('getTempPath');

            $fakeContext = $this->getFakeContext(SerializationFormat::Xml,SerializationFormat::Xml);
            $fakeContext->IppConfiguration->ContentWriter->strategy = "file";
            $i = new DataServiceMock($fakeContext);
            $i->processDownloadedContent($contentWriterMock,200);

         }

         public function testDownloadPDF_ProcessingHandler()
         {
            $contentWriterMock = $this->getMock("ContentWriter");
            $contentWriterMock->method('isHandler')->will($this->returnValue(true));
            $contentWriterMock->expects($this->once())
                            ->method('saveAsHandler');
            $contentWriterMock->expects($this->once())
                            ->method('getHandler');
            $fakeContext = $this->getFakeContext(SerializationFormat::Xml,SerializationFormat::Xml);
            $fakeContext->IppConfiguration->ContentWriter->strategy = "handler";
            $i = new DataServiceMock($fakeContext);
            $i->processDownloadedContent($contentWriterMock,200);

         }

         public function testDownloadPDF_ProcessingExport()
         {
            $fakeFileName  =  "test.name";
            $fakeExportDir =  "e/x/p/o/r/t";
            $contentWriterMock = $this->getMock("ContentWriter");
            //$contentWriterMock->method('isFileExport')->will($this->returnValue(true));
            $contentWriterMock->expects($this->once())
                            ->method('saveFile')
                            ->with(
                                    $this->equalTo($fakeExportDir),
                                    $this->equalTo($fakeFileName)
                                   );
            $contentWriterMock->expects($this->once())
                            ->method('getTempPath');
            $fakeContext = $this->getFakeContext(SerializationFormat::Xml,SerializationFormat::Xml);
            $fakeContext->IppConfiguration->ContentWriter->strategy = "export";
            $fakeContext->IppConfiguration->ContentWriter->exportDir = $fakeExportDir;
            $i = new DataServiceMock($fakeContext);
            $i->processDownloadedContent($contentWriterMock,200,$fakeFileName);

         }


         public function testDownloadPDF_ProcessingReturn()
         {
            $contentWriterMock = $this->getMock("ContentWriter");
            $fakeContext = $this->getFakeContext(SerializationFormat::Xml,SerializationFormat::Xml);
            $i = new DataServiceMock($fakeContext);
            $fakeContext->IppConfiguration->ContentWriter->returnOject = true;
            $this->assertEquals($contentWriterMock,$i->processDownloadedContent($contentWriterMock,200));
         }

         public function testDownloadPDF_ProcessingException()
         {
            $contentWriterMock = $this->getMock("ContentWriter");
            $contentWriterMock->method('resetContent')->will($this->throwException(new Exception));
            $contentWriterMock->expects($this->never())->method('isHandler');
            $fakeContext = $this->getFakeContext(SerializationFormat::Xml,SerializationFormat::Xml);
            $i = new DataServiceMock($fakeContext);
            $this->assertNull($i->processDownloadedContent($contentWriterMock,200));
         }
        ...
        /**
         ...
         public function testGetExportFileNameForPDF()
         {
            $e = new IPPTaxService();
            $e->Id = "123";
            $fakeContext = $this->getFakeContext(SerializationFormat::Xml,SerializationFormat::Xml);
            ...
            $this->assertRegExp("/taxservice_123_\d+\.ext/",$i->getExportFileNameForPDF($e, 'ext'));
